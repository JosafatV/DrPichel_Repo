...
namespace DrPhischelWebApi.DataBase
{
    public class DoctorAccess
    {
        ...
        public List<Cobros> getCobros(string Fecha)
        {
            List<Cobros> ListDoctores = new List<Cobros>();
            string cs = ConfigurationManager.ConnectionStrings["DBCS"].ConnectionString;
            using (SqlConnection con = new SqlConnection(cs))
            {
                SqlCommand cmd = new SqlCommand(
                "exec SP_Cobro_Doctores  @FechaRequested = '" + Fecha + "'"
                ...
                while (rdr.Read()) //si existe en la base de datos
                {
                    Cobros doctor = new Cobros();
                    doctor.MontoAlCobro = rdr["MontoAlCobro"].ToString();
                    doctor.NoDoctor = rdr["NoDoctor"].ToString();
                    doctor.NumeroPacientes = rdr["NumeroPacientes"].ToString();
                    ListDoctores.Add(doctor);
